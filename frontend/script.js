document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateResult(category) {
      ...
      resultValue.textContent = `${val} ${siBaseUnits[category]} = ${bijuVal.toFixed(3)} Biju ${category} units`;
    }
    ...
    categories.forEach(category => {
      ...
      box.addEventListener('click', () => {
        if(inputValue.value.trim() === '') {
          resultValue.textContent = `Enter value in ${siBaseUnits[category]} and click the dimension again to convert.`;
          return;
        }
